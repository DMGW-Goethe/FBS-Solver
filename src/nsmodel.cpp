 ...
 * */
vector NSmodel::dy_dr_static(const NUMERIC r, const vector& y, const void* params) {
 ...
 * */
int NSmodel::integrate(std::vector<integrator::step>& result, std::vector<integrator::Event>& events, const vector initial_conditions, integrator::IntegrationOptions intOpts, NUMERIC r_init, NUMERIC r_end) const {
    return integrator::RKF45(&(this->dy_dr_static), (r_init < 0._num ? this->r_init : r_init), initial_conditions, (r_end < 0._num ? this->r_end : r_end), (void*) this,  result,  events, intOpts);
}
