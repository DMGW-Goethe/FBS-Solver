                            ...
                            std::vector<step>& results, std::vector<step>& events, const bool save_intermediate, event_condition event, event_condition stopping_condition)
{
    ...
    RK45::step current_step = std::make_pair(r0, y0);
    results.empty(); events.empty();
    ...
    while(true) {
        ...
        if ( event != NULL && event(current_step.first, current_step.second, params))
            events.push_back(current_step);

        if ( stopping_condition != NULL &&  stopping_condition(current_step.first, current_step.second, params) )
            stop = 3;
